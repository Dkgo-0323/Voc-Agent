...
CASES = (
    ...
    _case("SG44", GoldenCategory.ABSTENTION, "Give me data for an unknown power station SKU.", required=(Tool.SQL,), allowed=(Tool.SQL,), abstain=True, answer=("Surface the safe invalid-SKU outcome and make no product claims.",), notes="Unknown or disabled SKUs are rejected before deterministic queries."),
    ...
    _case("SG46", GoldenCategory.ABSTENTION, "Compare Delta 2 with Anker F2000 and choose the winner.", required=(Tool.SQL,), allowed=(Tool.SQL,), skus=(DELTA_2, ANKER_F2000), abstain=True, answer=("Reject the cross-tier request and never name a winner.",), notes="Cross-tier comparison is an application-enforced constraint, not an LLM preference."),
    ...
    _case("SG49", GoldenCategory.REPORT, "Is there a stored report for Delta 2 in invalid week 202453?", required=(Tool.REPORT,), allowed=(Tool.REPORT,), skus=(DELTA_2,), weeks=(202453,), abstain=True, answer=("Return the safe invalid-week result; do not generate a report.",), notes="Stored report lookup validates ISO week before repository access."),
