...
export function ReportDashboard() {
  ...
  const startGeneration = async () => {
    if (!skuCode || !weekId || activeGeneration) return;
    ...
    try {
      await streamReportGeneration({ sku_code: skuCode, week_id: weekId }, (event: ReportStreamEvent) => {
        ...
        if (event.event_type === "error") streamError = event.error.message;
      });
      if (streamError) { setGenerationError(streamError); setGenerationState("error"); } else setGenerationState("idle");
    } catch (caught) {
      setGenerationError(toApiError(caught).message); setGenerationState("error");
    }
  };
